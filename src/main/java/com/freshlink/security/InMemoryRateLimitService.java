 ...
 *
 * Limits are therefore per-instance: two replicas double the effective
 * allowance. Fine for a single deployment, and the default so that running
 * locally needs no extra infrastructure - set {@code app.ratelimit.backend=redis}
 * to share buckets across instances instead.
 */
@Service
